...
namespace GaussianSplatting.Runtime
{
    class GaussianSplatRenderSystem
    {
        ...
        public Material SortAndRenderSplats(Camera cam, CommandBuffer cmb)
        {
            ...
            foreach (var kvp in m_ActiveSplats)
            {
                ...
                var mpb = kvp.Item2; //mpb:material property block
    ...
    public class GaussianSplatRenderer : MonoBehaviour
    {
        ...
        GraphicsBuffer m_GpuCustomMeshVertices; // Buffer to hold custom mesh vertices
        GraphicsBuffer m_GpuCustomMeshIndices;
        ...
        internal static class Props
        {
            ...
            public static readonly int CustomMeshVertices = Shader.PropertyToID("_CustomMeshVertices");
            public static readonly int CustomMeshIndices = Shader.PropertyToID("_CustomMeshIndices");
        ...
        private Vector2 dragStartPosition; // Store the mouse position when the drag starts
        private bool isDragging = false;
        private Vector3 initialMouseWorldPosition; // To store the initial mouse world position during right-click dragging
        private bool isRightClickDragging = false; // Track right-click dragging state
        ...
        void SetAssetDataOnCS(CommandBuffer cmb, KernelIndices kernel)
        {
            ...
            if (m_Cutouts != null)
            {
                foreach (var cutout in m_Cutouts)
                {
                    if (cutout.m_Type == GaussianCutout.Type.CustomMesh && cutout.m_CustomMesh != null)
                    {
                        var vertices = cutout.m_CustomMesh.vertices;
                        var triangles = cutout.m_CustomMesh.triangles;

                        if (m_GpuCustomMeshVertices == null || m_GpuCustomMeshVertices.count != vertices.Length)
                        {
                            
                            m_GpuCustomMeshVertices?.Dispose();
                            m_GpuCustomMeshVertices = new GraphicsBuffer(GraphicsBuffer.Target.Structured, vertices.Length, sizeof(float) * 3);
                            m_GpuCustomMeshVertices.SetData(vertices);
                        }
                        //UnityEngine.Debug.Log($"Vertex: {vertices[0]}");
                        cmb.SetComputeBufferParam(cs, kernelIndex, Props.CustomMeshVertices, m_GpuCustomMeshVertices);
                        cmb.SetComputeIntParam(cs, "_CustomMeshVertexCount", vertices.Length);

                        if (m_GpuCustomMeshIndices == null || m_GpuCustomMeshIndices.count != triangles.Length)
                        {
                            m_GpuCustomMeshIndices?.Dispose();
                            m_GpuCustomMeshIndices = new GraphicsBuffer(GraphicsBuffer.Target.Structured, triangles.Length, sizeof(int));
                            m_GpuCustomMeshIndices.SetData(triangles);
                        }

                        // Send triangle index buffer to the GPU
                        cmb.SetComputeBufferParam(cs, kernelIndex, Props.CustomMeshIndices, m_GpuCustomMeshIndices);
                        cmb.SetComputeIntParam(cs, "_CustomMeshTriangleCount", triangles.Length / 3);
                    }
                }
            }
        }
        ...
        internal void SetAssetDataOnMaterial(MaterialPropertyBlock mat)
        {
            ...
            if (m_Cutouts != null)
            {
                foreach (var cutout in m_Cutouts)
                {
                    if (cutout.m_Type == GaussianCutout.Type.CustomMesh && cutout.m_CustomMesh != null)
                    {
                        int vertices = cutout.m_CustomMesh.vertices.Length;
                        int triangles = cutout.m_CustomMesh.triangles.Length / 3;
                        //UnityEngine.Debug.Log($"Vertex: {vertices}");
                        mat.SetBuffer(Props.CustomMeshVertices, m_GpuCustomMeshVertices);
                        mat.SetInteger("_CustomMeshVertexCount", vertices);
                        mat.SetBuffer(Props.CustomMeshIndices, m_GpuCustomMeshIndices);
                        mat.SetInteger("_CustomMeshTriangleCount", triangles);
                    }
                }
            }
        ...
        void DisposeResourcesForAsset()
        {
            ...
            DisposeBuffer(ref m_GpuCustomMeshVertices);
            DisposeBuffer(ref m_GpuCustomMeshIndices);
        ...
        private void HandleLeftClickSelection()
        {
            ...
            {
                dragStartPosition = Input.mousePosition; // Store the start position in screen space
                isDragging = true;
            }

            // If the mouse button is held down and dragging is active, update the selection
            if (isDragging && Input.GetMouseButton(0))
            {
                Vector2 currentMousePosition = Input.mousePosition; // Get the current mouse position
                EditUpdateSelection(dragStartPosition, currentMousePosition, Camera.main, false);
            }
        ...
        // Function to handle right-click dragging to move selected splats
        private void HandleRightClickMovement()
        {
            // Start dragging when the right mouse button is pressed
            if (Input.GetMouseButtonDown(1) && IsSplatSelected())
            {
                initialMouseWorldPosition = GetMouseWorldPosition(); // Store the initial mouse world position
                isRightClickDragging = true;
            }

            // If right mouse button is held down, move the selected splats
            if (isRightClickDragging && Input.GetMouseButton(1))
            {
                Vector3 currentMouseWorldPosition = GetMouseWorldPosition(); // Get the current mouse world position
                Vector3 delta = currentMouseWorldPosition - initialMouseWorldPosition; // Calculate movement delta

                EditTranslateSelection(delta); // Move the splats
                initialMouseWorldPosition = currentMouseWorldPosition; // Update the initial position for continuous dragging
            }

            // Stop dragging when the right mouse button is released
            if (Input.GetMouseButtonUp(1))
            {
                isRightClickDragging = false; // Stop moving the splats
            }
        }

        private void HandleSpaceKeyForCopy()
        {
            // Check if space key is pressed and splats are selected
            if (Input.GetKeyDown(KeyCode.Space) && IsSplatSelected())
            {
                
                //CopySelectedSplats(); // Call function to copy splats
        ...
        private Vector3 GetMouseWorldPosition()
        {
            ...
            Plane groundPlane = new Plane(Vector3.up, Vector3.zero); // Example: ground plane at y = 0
            float distance;

            // Find the intersection point of the ray and the plane
            if (groundPlane.Raycast(ray, out distance))
            {
                return ray.GetPoint(distance); // Get the world position where the ray intersects the plane
            }

            return Vector3.zero; // Return zero if no intersection is found
        }

        // Check if any splat is selected
        private bool IsSplatSelected()
        {
            // This function should check if any splat is selected
